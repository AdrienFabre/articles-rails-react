...
RSpec.describe Article do
  ...
  subject {described_class.new(id: 1138391, likes: 0)}
  ...
  describe '#complete_article' do 
    it 'returns an object with necessary keys' do
      expect(subject.complete_article(full_article_data).keys).to contain_exactly(:id, :likes, :title, :description, :image, :location, :user,:user_avatar, :updated_at)
    end
  end
